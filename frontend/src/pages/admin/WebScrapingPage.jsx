...
export const WebScrapingPage = () => {
  ...
  const handlePreview = async (url) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/web-scraping/preview`, {
        url: url,
      });
  ...
  const handleUpdateSource = async () => {
    try {
      ...
      await axios.put(`${API_BASE_URL}/web-scraping/sources/${editingSource.id}`, {
        name: newSource.name,
        url: newSource.url,
        description: newSource.description,
        keywords: keywords,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        >
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats?.total_scrapes || 0}</div>
              <p className="text-xs text-muted-foreground">
        ...
        {stats?.filtering_stats && stats.filtering_stats.scrapes_with_keywords > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Filter Match Rate</CardTitle>
                <Zap className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {stats.filtering_stats.average_match_rate_percent?.toFixed(1) || 0}%
                </div>
                <p className="text-xs text-muted-foreground">
                  {stats.filtering_stats.total_documents_skipped || 0} filtered out
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <DialogContent className="sm:max-w-[525px]">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Only documents containing these keywords will be scraped. Leave empty to scrape all documents.
                </p>
              </div>
              <div className="grid gap-2">
          ...
          <DialogContent className="sm:max-w-[525px]">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Change keywords to filter different types of documents. Leave empty to scrape all documents.
                </p>
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-max_docs">Max Documents per Scrape</Label>
                <Input
                  id="edit-max_docs"
                  type="number"
                  value={newSource.max_documents}
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {source.last_scraped_at && (
                          <span className="flex items-center gap-1">
                            ...
                            Last: {new Date(source.last_scraped_at).toLocaleString()}
                          </span>
                        )}
                        {source.last_scrape_status && (
                          <Badge
                            variant="outline"
                            className={getStatusColor(source.last_scrape_status)}
                          >
                            {source.last_scrape_status}
                          </Badge>
                        )}
                ...
                {logs.map((log) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <p className="font-medium text-sm">{log.source_name}</p>
                        <p className="text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {log.documents_matched || log.documents_found || 0} docs
                        </Badge>
                        {log.status === "success" ? (
                      ...
                      <div className="mt-2 flex items-center gap-2 flex-wrap">
                        ...
                        {log.documents_discovered > 0 && (
                          <span className="text-xs text-muted-foreground">
                            ({log.documents_matched}/{log.documents_discovered} matched)
                          </span>
                        )}
                      </div>
                    )}
              ...
              <div className="mb-4 space-y-2">
                ...
                {searchKeyword && (
                  <p className="text-sm text-muted-foreground">
                    Showing {scrapedDocs.filter((doc) => {
                    ...
                    }).length} of {scrapedDocs.length} documents
                  </p>
                )}
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        {doc.matched_keywords && doc.matched_keywords.length > 0 && (
                          <div className="flex items-center gap-1 flex-wrap">
                            {doc.matched_keywords.map((keyword, kidx) => (
                              <Badge 
                                key={kidx} 
                                variant="secondary" 
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const downloadUrl = `${API_BASE_URL}/web-scraping/download-document?url=${encodeURIComponent(doc.url)}`;
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">{previewData.page_title || "N/A"}</p>
                </div>
                <div>
              ...
              <div>
                <Label>Sample Documents ({previewData.sample_documents})</Label>
